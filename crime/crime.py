...
class Solution(Reader):
    def __init__(self):  # init 생성자로서 안에 두면 객체가 된다. 데이터를 공유하기 위해서는 init에 둬야 한다 (내장 함수,함수는 )
        self.file = File()
        # self.reader = Reader(
    ...
    def hook(self):
        def print_menu():
            print('0. Exit')
            print('1. crime_in_seoul.csv, 구글맵 API 를 이용 해서 서울시 내 경찰서 주소목록파일을 작성하시오.')
            print('2. us-states.json, us_unemployment.csv 를 이용 해서 미국 실업률 지도를 작성하시오.')
            print('3. cctv_in_seoul.csv, pop_in_seoul.csv 를 이용해서 서울시 내 경찰서 주소목록파일(cctv_pop.csv)을 작성하시오.')
    ...
    def save_police_pos(self):  # 메서드
        ...
        # print(f'station_names range: {len(station_names)}') #서울시내 경찰서는 31개다.
        ...
        print(a)
        # 종암경찰서는 null값으로 나온다. 종암경찰서가 임시청사로 이전 했기때문에(구글맵에서 주소가바뀜)
        '''
        ...
        for i, name in enumerate(station_names):
            ...
            '''
            0번 중부서인경우는 "대한민국 서울 특별시 중구 수표로 27"이 담긴다.
    ...
    def save_cctv_pos(self):
        ...
        skiprows = [2]
        header = [2]
        pop = self.xls(file, header, cols)  # 헤더는 2행, 사용 하는 컬럼은 B D G J N 이다.
        # print(cctv)
        print('*' * 100)
        # print(pop)
        '''
        ...
        # pop.dropna(how='all') #한개라도 Nan값일시 날림
        pop.drop(26, axis=0, inplace=True)
        ...
        # print(cctv)
        cctv_pop = pd.merge(pop, cctv, on='구별')
        cor1 = cctv_pop['고령자비율'].corr(cctv_pop['소계'], method='kendall')
        cor2 = cctv_pop['외국인비율'].corr(cctv_pop['소계'], method='kendall')
    ...
    def save_police_norm(self):  # 정규화 리스케일링
        file = self.file
        file.context = './data/'
        file.fname = 'crime_in_seoul'
